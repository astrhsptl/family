...
class BaseService:
    ...
    async def get_all(
        ...
        result = await cls._repository.get_all(page, quantity, order_by)

        if result.error:
            return ErrorResponse(detail=result.detail, status_code=result.status_code)

        return cls._paginate(request, result.count, result.data, page, quantity)
    ...
    @classmethod
    def _paginate(cls, request: Request, count: int, data, page: int, quantity: int):
        page = max(page, 1)
        url = request.url
        ...
        return PaginatedResponse[cls._data_serializer](
